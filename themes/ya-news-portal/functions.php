...
add_action( 'admin_notices', 'yanews_notice' );
function yanews_notice() {
$user_id = get_current_user_id();
$admin_url = ( isset( $_SERVER['HTTPS'] ) && $_SERVER['HTTPS'] === 'on' ? 'https' : 'http' ) . "://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
$param = ( count( $_GET ) ) ? '&' : '?';
if ( !get_user_meta( $user_id, 'yanews_notice_dismissed_12' ) && current_user_can( 'manage_options' ) )
echo '<div class="notice notice-info"><p><a href="' . esc_url( $admin_url ), esc_html( $param ) . 'dismiss" class="alignright" style="text-decoration:none"><big>' . esc_html__( 'Ⓧ', 'yanews' ) . '</big></a>' . wp_kses_post( __( '<big><strong>🏆 Thank you for using yanews!</strong></big>', 'yanews' ) ) . '<p>' . esc_html__( 'Powering over 10k websites! Buy me a sandwich! 🥪', 'yanews' ) . '</p><a href="https://github.com/webguyio/yanews/issues/57" class="button-primary" target="_blank"><strong>' . esc_html__( 'How do you use yanews?', 'yanews' ) . '</strong></a> <a href="https://opencollective.com/yanews" class="button-primary" style="background-color:green;border-color:green" target="_blank"><strong>' . esc_html__( 'Donate', 'yanews' ) . '</strong></a> <a href="https://wordpress.org/support/theme/yanews/reviews/#new-post" class="button-primary" style="background-color:purple;border-color:purple" target="_blank"><strong>' . esc_html__( 'Review', 'yanews' ) . '</strong></a> <a href="https://github.com/webguyio/yanews/issues" class="button-primary" style="background-color:orange;border-color:orange" target="_blank"><strong>' . esc_html__( 'Support', 'yanews' ) . '</strong></a></p></div>';
}
add_action( 'admin_init', 'yanews_notice_dismissed' );
function yanews_notice_dismissed() {
$user_id = get_current_user_id();
if ( isset( $_GET['dismiss'] ) )
add_user_meta( $user_id, 'yanews_notice_dismissed_12', 'true', true );
}
add_action( 'wp_enqueue_scripts', 'yanews_enqueue' );
function yanews_enqueue() {
wp_enqueue_style( 'yanews-style', get_stylesheet_uri() );
...
add_filter( 'intermediate_image_sizes_advanced', 'yanews_image_insert_override' );
function yanews_image_insert_override( $sizes ) {
unset( $sizes['medium_large'] );
unset( $sizes['1536x1536'] );
unset( $sizes['2048x2048'] );
return $sizes;
}
add_action( 'widgets_init', 'yanews_widgets_init' );
function yanews_widgets_init() {
register_sidebar( array(
'name' => esc_html__( 'Sidebar Widget Area', 'yanews' ),
